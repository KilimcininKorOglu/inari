# Inari Benchmark Harness

Measures whether Inari CLI actually reduces token consumption and improves task correctness for LLM coding agents.
...
|-----------|-------------|
| **without-inari** | Agent works without Inari CLI (tools disallowed via `Bash(inari:*)` glob) |
| **with-inari** | Agent has Inari CLI available |
| **with-inari-preloaded** | Agent has Inari CLI AND `inari map` output baked into CLAUDE.md |
...
- **Inari CLI** — `cargo install --path .` from the repo root
...
# 2. Install inari globally (needed for with-inari and preloaded conditions)
cd ../..
cargo install --path .
inari --version    # Should print inari 0.7.2+
...
Runs 1 task across all 3 conditions (3 agent invocations). Prints PASS/FAIL per condition, validating that tokens, actions, file reads, inari commands, and NDJSON streams are all captured. **Fix any failures before proceeding to a full run.**
...
- `Actions: > 0`
- `File reads: > 0`
- `Inari commands: [inari find]` (for with-inari conditions)
- `Telemetry: VALID`
...
| `--compare` | Run with-inari + without-inari (2-arm) | - |
| `--conditions 3` | Run all 3 arms | 1 |
| `--no-inari` | Run baseline only | - |
| `--inari-only` | Run with-inari only | - |
...
| `--compare` | Prepare with-inari + without-inari (2-arm) | - |
...
benchmark verify --dir ./prepared/ts-cat-b-01-with-inari

# JSON output
benchmark verify --dir ./prepared/ts-cat-b-01-with-inari --json
...
benchmarks/
  ...
  prepared/
    phaseNN/
      ...
      ts-cat-a-01-with-inari/  # Isolated work directory
      ts-cat-a-01-without-inari/
      ...
...
- `CLAUDE.md.with-inari` / `CLAUDE.md.without-inari` -- swapped per condition
- `CLAUDE.md.with-inari-preloaded` -- template with `{{SCOPE_MAP_OUTPUT}}` placeholder
- `.inari/` -- pre-built Inari index
